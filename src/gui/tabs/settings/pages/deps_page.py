...
class FFmpegOptionsPanel(QFrame):
    ...
    def _build_ui(self):
        ...
        mode_grid.addWidget(mode_lbl, 0, 0)

        self._group_mode = QButtonGroup(self)
        self._radio_managed = self._make_radio(self.tr("Gestionado por DowP (Recomendado)"), "managed")
        self._radio_custom = self._make_radio(self.tr("FFmpeg Personalizado (Ruta local)"), "custom")
        ...
        mode_grid.addWidget(self._radio_managed, 0, 1)
        mode_grid.addWidget(self._radio_custom, 0, 2)
        mode_grid.setColumnMinimumWidth(1, 290)
        mode_grid.setColumnMinimumWidth(2, 230)
        ...
        managed_grid = QGridLayout()
        managed_grid.setContentsMargins(0, 0, 0, 0)
        managed_grid.setHorizontalSpacing(16)
        managed_grid.setVerticalSpacing(8)

        # Fila 0: Variante
        v_lbl = QLabel(self.tr("Compilación:"))
        v_lbl.setFixedWidth(90)
        v_lbl.setStyleSheet("color: #CCC; font-size: 12px;")
        managed_grid.addWidget(v_lbl, 0, 0)

        self._group_variant = QButtonGroup(self)
        self._radio_essentials = self._make_radio(self.tr("Essentials (Ligero, códecs estándar)"), "essentials")
        self._radio_full = self._make_radio(self.tr("Full (Códecs extendidos y librerías completas)"), "full")
        ...
        managed_grid.addWidget(self._radio_essentials, 0, 1)
        managed_grid.addWidget(self._radio_full, 0, 2)

        # Fila 1: Canal / Versión
        c_lbl = QLabel(self.tr("Canal:"))
        c_lbl.setFixedWidth(90)
        ...
        managed_grid.addWidget(self._radio_recommended, 1, 1)
        managed_grid.addWidget(self._radio_latest, 1, 2)
        managed_grid.addWidget(self._radio_nightly, 1, 3)

        # Fila 2: Checkbox ffplay (inicia en columna 1 alineado)
        self._chk_keep_ffplay = QCheckBox(self.tr("Conservar ffplay.exe (Reproductor por terminal, ~70 MB extra)"))
        self._chk_keep_ffplay.toggled.connect(self._on_keep_ffplay_toggled)
        managed_grid.addWidget(self._chk_keep_ffplay, 2, 1, 1, 3)

        managed_grid.setColumnMinimumWidth(1, 290)
        managed_grid.setColumnMinimumWidth(2, 230)
        managed_grid.setColumnStretch(4, 1)
        managed_layout.addLayout(managed_grid)
